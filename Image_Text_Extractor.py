...
def est_extract():
    ...
    for ptry in range(1,4):
        if breaking:
            break
        for qtry in range(1,4):
            message_bit_array = est_extract_bits_from_image(
            image_byte_array, header_len, ptry, qtry )
            if breaking:
                break
            proportion = est_bit_proportion(message_bit_array)
            print("p="+str(ptry)+" q="+str(qtry)+" bp="+"{0:.3f}".format(proportion))
            if proportion<0.5:
                print("The answer is: p="+str(ptry))
                print("q="+str(qtry))
                p=ptry
                q=qtry
                breaking=True
...
def est_bit_proportion(text_as_bits):
    proportion = 0.0
    ones = 0
    notones = 0
    for i in range( 0, len(text_as_bits)):
        if text_as_bits[i]==1:
            ones+=1
    proportion = float(ones)/len(text_as_bits)
    return proportion
